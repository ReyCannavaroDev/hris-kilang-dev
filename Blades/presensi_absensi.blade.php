...
<div class="bg-white p-6 rounded-md">
  ...
  <h3 v-if="openDateSelected" class="font-semibold mb-2">Tanggal : {{openDateSelected}}</h3>
  <div class="overflow-auto w-full rounded-lg">
  <table class="table-auto w-full " v-if="openDateSelected && dataByDateDetail.length">
    <thead class="bg-gray-500  font-semibold text-white text-[12px] ">
      <tr>
        ...
        <th class="border-1 border-gray-500 px-3 py-2 w-[22%]">Status</th>
        <th class="border-1 border-gray-500 px-3 py-2 w-[9%]">Waktu Checkin</th>
        <th class="border-1 border-gray-500 px-3 py-2 w-[9%]">Waktu Break Out</th>
        <th class="border-1 border-gray-500 px-3 py-2 w-[9%]">Waktu Resume Work</th>
        <th class="border-1 border-gray-500 px-3 py-2 w-[9%]">Waktu Checkout</th>
        <th class="border-1 border-gray-500 px-3 py-2 w-[50px]">Aksi</th>
        <th class="border-1 border-gray-500 px-3 py-2 w-[50px]"></th>
        <th class="border-1 border-gray-500 px-3 py-2">Created At</th>
        <th class="border-1 border-gray-500 px-3 py-2">Updated At</th>
        <th class="border-1 border-gray-500 px-3 py-2">Updated By</th>
      </tr>
    </thead>
    <tbody class="bg-blue-100" v-if="openDateSelected && dataByDateDetail.length">
      <tr class="hover:bg-amber-200 cursor-pointer" v-for="item,idx in dataByDateDetail" :key="idx"
      :class="[
  item?.absensi?.status === 'NOT ATTEND' ? 'bg-red-200' : '',
  item?.absensi?.status === 'OFF' ? 'bg-green-200' : '',
  item?.absensi?.status === 'CUTI' ? 'bg-orange-500' : ''
]"

 class="hover:bg-amber-200; text-[11px]">
        <!-- {{$log(item)}} -->
        <td class="text-left border-1 text-center border-gray-500 px-3">{{idx+1}}</td>
        ...
        <td class="text-left border-1 border-gray-500 px-3 cursor-pointer"
          @click="openDateDetail(true,item.absensi,'checkin',item)">
          <div class="flex items-center">
            <icon v-show="item.absensi.checkout_istirahat_time" class="mx-1"
              :class="item.absensi?.checkin_on_scope?'text-green-600':'text-red-600'" fa="map-marker-alt" />
            {{item.absensi.checkout_istirahat_time}}
          </div>
        </td>
        ...
        <td class="text-left border-1 border-gray-500 px-3 cursor-pointer"
          @click="openDateDetail(true,item.absensi,'checkin',item)">
          <div class="flex items-center">
            <icon v-show="item.absensi?.checkin_kerja_time" class="mx-1"
              :class="item.absensi?.checkin_on_scope?'text-green-600':'text-red-600'" fa="map-marker-alt" />
            {{item.absensi?.checkin_kerja_time}}
          </div>
        </td>
        ...
        <td class="text-center border-1 border-gray-500 px-3">
          <button
            @click="removeDetail(item.absensi?.presensi_absensi_id)"class="text-gray-600">
            <icon fa="trash" />
          </button>
        </td>
<td class="text-center border-1 border-gray-500 px-3">
  {{item.absensi.created_at.toLocaleString() }}
</td>
<td class="text-center border-1 border-gray-500 px-3">
  {{item.absensi?.updated_at.toLocaleString() }}
</td>
<td class="text-center border-1 border-gray-500 px-3">
  {{ item.absensi?.last_editor_nama }}
</td>
...
<div v-if="showModal" class="fixed inset-0 flex items-center justify-center z-50" id="modal">
  ...
  <div class="bg-white w-[50%] rounded shadow-lg z-10 overflow-auto max-h-[70%]">
    ...
    <div class="grid grid-cols-8 md:grid-cols-12 text-[14px] gap-x-[29px] gap-y-[20px] px-[30px] py-[27px]">
      ...
      <div class="col-span-8 md:col-span-9 mt-2 mb-2">
        <table >
          ...
          <tr>
            <td class="w-[30%] py-1 px-2">Waktu Istirahat</td>
            <td class="align-top py-1 px-1">:</td>
            <td class=" py-1 px-1">{{dataDetail.checkout_istirahat_time}}</td>
          </tr>
          <tr>
            <td class="w-[30%] py-1 px-2">Mulai Kerja</td>
            <td class="align-top py-1 px-1">:</td>
            <td class=" py-1 px-1">{{dataDetail.checkin_kerja_time}}</td>
          </tr>
          <tr>
  ...
  <div class="flex gap-x-4 px-2">
    <div class="flex flex-col border rounded shadow-sm px-6 py-6 <md:w-full w-full bg-white">
      ...
      <div class="grid <md:grid-cols-1 grid-cols-2 gap-x-[80px] gap-y-2">
        ...
        <div>
          <label class="font-semibold">Missing Check</span></label>
          <FieldNumber :bind="{ readonly: !actionText }" label="" class="w-full py-2 !mt-0" :value="values.missing_check"
            :errorText="formErrors.missing_check ? 'failed' : ''" @input="v=>values.missing_check=v" :hints="formErrors.missing_check"
            :check="false" label="" placeholder="Missing Check" />
        </div>
        ...
        <div class="space-y-4">
          <div class="mt-2 sm:mt-6">
            <p class="font-bold text-[18px]">Check Out Istirahat</p>
          </div>
          <div class="">
            <label class="font-semibold">Waktu Check Out Istirahat</span></label>
            <FieldX :bind="{ readonly: !actionText }" type="time" label="" class="w-full py-2 !mt-0"
              :value="values.checkout_istirahat_time" :errorText="formErrors.checkout_istirahat_time ? 'failed' : ''"
              @input="v=>values.checkout_istirahat_time=v" :hints="formErrors.checkout_istirahat_time" :check="false"
              label="" placeholder="Waktu Check Out Istirahat" />
          </div>

          <div>
            <label class="font-semibold">Foto Check Out Istirahat</span></label>
            <div class="rounded-md py-2 h-[370px]">
              <img v-if="!isCheckin && values.checkout_istirahat_foto" :src="values.checkout_istirahat_foto" alt="Check Out Istirahat Photo" class="w-[200px] h-[100%] rounded-md object-center">
              <div v-else class="h-full bg-gray-500 w-[200px] rounded-[10px]"></div>
            </div>
          </div>

          <div v-show="isCreateEdit">
            <label class="font-semibold">Lokasi Check Out Istirahat</span></label>
            <FieldSelect class="w-full py-2 !mt-0" :bind="{ disabled: !actionText, clearable:false }"
              :value="values.checkout_istirahat_region" @input="v=>values.checkout_istirahat_region=v"
              :errorText="formErrors.checkout_istirahat_region?'failed':''"
              :hints="formErrors.checkout_istirahat_region" @update:valueFull="(objVal)=>{
              $log(objVal)
              values.geo_checkin = `POINT(${objVal.long} ${objVal.lat})`
              values.checkin_long = objVal.long
              values.checkin_lat = objVal.lat
          ...
          <div v-show="isCreateEdit">
            <label class="font-semibold">Pin Titik Lokasi Check Out Istirahat</span></label>
            <FieldGeo class="w-full py-2 !mt-0" :bind="{ readonly: !actionText }" @input="(v)=>{
            values.geo_checkin=v
            values.test = values.geo_checkin?.match(/\(([^)]+)\)/)[1].split(' ')
            if(values.test){
              values.checkout_istirahat_long = values.test[0]
              values.checkout_istirahat_lat = values.test[1]
            }
          }" :center="[-7.3244677, 112.7550714]" :errorText="formErrors.geo_checkin?'failed':''"
              :hints="formErrors.geo_checkin" geostring="POINT(112.7550714 -7.3244677)" :value="values.geo_checkin"
              placeholder="Pilih Titik Lokasi" fa-icon="map-marker-alt" :check="false" />
          ...
          <div>
            <label class="font-semibold">Latitude Check Out Istirahat</span></label>
            <FieldX :bind="{ readonly: true }" label="" class="w-full py-2 !mt-0" :value="values.checkout_istirahat_lat"
              :errorText="formErrors.checkout_istirahat_lat ? 'failed' : ''" @input="v=>values.checkout_istirahat_lat=v"
              :hints="formErrors.checkout_istirahat_lat" :check="false" label=""
              placeholder="Latitude Check Out Istirahat" />
          </div>
          <div>
            <label class="font-semibold">Longtitude Check Out Istirahat</span></label>
            <FieldX :bind="{ readonly: true }" label="" class="w-full py-2 !mt-0"
              :value="values.checkout_istirahat_long" :errorText="formErrors.checkout_istirahat_long ? 'failed' : ''"
              @input="v=>values.checkout_istirahat_long=v" :hints="formErrors.checkout_istirahat_long" :check="false"
              label="" placeholder="Longtitude Check Out Istirahat" />
          </div>
          <div>
            <label class="font-semibold">Alamat Check Out Istirahat</span></label>
            <FieldX :bind="{ readonly: !actionText }" type="textarea" label="" class="w-full py-2 !mt-0"
              :value="values.checkout_istirahat_address"
              :errorText="formErrors.checkout_istirahat_address ? 'failed' : ''"
              @input="v=>values.checkout_istirahat_address=v" :hints="formErrors.checkout_istirahat_address"
              :check="false" label="" placeholder="Alamat Check Out Istirahat" />
          </div>

          <div>
            <label class="font-semibold">Catatan Check Out Istirahat<span class="text-red-500 space-x-0 pl-0"></span></label>
            <FieldX :bind="{ readonly: !actionText }" type="textarea" label="" class="w-full py-2 !mt-0"
              :value="values.catatan_checkout_istirahat"
              :errorText="formErrors.catatan_checkout_istirahat ? 'failed' : ''"
              @input="v=>values.catatan_checkout_istirahat=v" :hints="formErrors.catatan_checkout_istirahat"
              :check="false" label="" placeholder="Catatan Check Out Istirahat" />
          </div>
          <div class="flex flex-col">
            <label class="font-semibold">On Scope<span class="text-red-500 space-x-0 pl-0">*</span></label>
            <div class="flex py-2 gap-2">
              <input :disabled="!actionText" type="radio" :value="true" v-model="values.checkout_istirahat_on_scope" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"/>
              <label for="ya">Ya</label>
              <div class="w-10 " />
              <input :disabled="!actionText" type="radio" v-model="values.checkout_istirahat_on_scope" :value="false" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"/>
              <label for="tidak">Tidak</label>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-4">
          <div class="mt-2 sm:mt-6">
            <p class="font-bold text-[18px]">Check In Istirahat</p>
          </div>
          <div class="">
            <label class="font-semibold">Waktu Check In Istirahat</span></label>
            <FieldX :bind="{ readonly: !actionText }" type="time" label="" class="w-full py-2 !mt-0"
              :value="values.checkin_kerja_time" :errorText="formErrors.checkin_kerja_time ? 'failed' : ''"
              @input="v=>values.checkin_kerja_time=v" :hints="formErrors.checkin_kerja_time" :check="false" label=""
              placeholder="Waktu Check In Istirahat" />
          </div>

          <div>
            <label class="font-semibold">Foto Check In Istirahat</span></label>
            <div class="rounded-md py-2 h-[370px]">
              <img v-if="!isCheckin && values.checkin_kerja_foto" :src="values.checkin_kerja_foto" alt="Check In Istirahat Photo" class="w-[200px] h-[100%] rounded-md object-center">
              <div v-else class="h-full bg-gray-500 w-[200px] rounded-[10px]"></div>
            </div>
          </div>

          <div v-show="isCreateEdit">
            <label class="font-semibold">Lokasi Check In Istirahat</span></label>
            <FieldSelect class="w-full py-2 !mt-0" :bind="{ disabled: !actionText, clearable:false }"
              :value="values.checkin_kerja_region" @input="v=>values.checkin_kerja_region=v"
              :errorText="formErrors.checkin_kerja_region?'failed':''" :hints="formErrors.checkin_kerja_region"
              @update:valueFull="(objVal)=>{
              $log(objVal)
              values.geo_checkin = `POINT(${objVal.long} ${objVal.lat})`
              values.checkin_kerja_long = objVal.long
              values.checkin_kerja_lat = objVal.lat
            }" label="" valueField="nama" displayField="nama" :api="{
                url: `${store.server.url_backend}/operation/presensi_lokasi`,
                headers: { 'Content-Type': 'Application/json', Authorization: `${store.user.token_type} ${store.user.token}`},
                params: {
          ...
          <div v-show="isCreateEdit">
            <label class="font-semibold">Pin Titik Lokasi Check In Istirahat</span></label>
            <FieldGeo class="w-full py-2 !mt-0" :bind="{ readonly: !actionText }" @input="(v)=>{
            values.geo_checkin=v
            values.test = values.geo_checkin?.match(/\(([^)]+)\)/)[1].split(' ')
            if(values.test){
              values.checkin_kerja_long = values.test[0]
              values.checkin_kerja_lat = values.test[1]
            }
          }" :center="[-7.3244677, 112.7550714]" :errorText="formErrors.geo_checkin?'failed':''"
              :hints="formErrors.geo_checkin" geostring="POINT(112.7550714 -7.3244677)" :value="values.geo_checkin"
              placeholder="Pilih Titik Lokasi" fa-icon="map-marker-alt" :check="false" />
          ...
          <div>
            <label class="font-semibold">Latitude Check In Istirahat</span></label>
            <FieldX :bind="{ readonly: true }" label="" class="w-full py-2 !mt-0" :value="values.checkin_kerja_lat"
              :errorText="formErrors.checkin_kerja_lat ? 'failed' : ''" @input="v=>values.checkin_kerja_lat=v"
              :hints="formErrors.checkin_kerja_lat" :check="false" label="" placeholder="Latitude Check In Istirahat" />
          </div>
          <div>
            <label class="font-semibold">Longtitude Check In Istirahat</span></label>
            <FieldX :bind="{ readonly: true }" label="" class="w-full py-2 !mt-0" :value="values.checkin_kerja_long"
              :errorText="formErrors.checkin_kerja_long ? 'failed' : ''" @input="v=>values.checkin_kerja_long=v"
              :hints="formErrors.checkin_kerja_long" :check="false" label=""
              placeholder="Longtitude Check In Istirahat" />
          </div>
          <div>
            <label class="font-semibold">Alamat Check In Istirahat</span></label>
            <FieldX :bind="{ readonly: !actionText }" type="textarea" label="" class="w-full py-2 !mt-0"
              :value="values.checkin_kerja_address" :errorText="formErrors.checkin_kerja_address ? 'failed' : ''"
              @input="v=>values.checkin_kerja_address=v" :hints="formErrors.checkin_kerja_address" :check="false"
              label="" placeholder="Alamat Check In Istirahat" />
          </div>

          <div>
            <label class="font-semibold">Catatan Check In Istirahat<span class="text-red-500 space-x-0 pl-0"></span></label>
            <FieldX :bind="{ readonly: !actionText }" type="textarea" label="" class="w-full py-2 !mt-0"
              :value="values.catatan_checkin_kerja" :errorText="formErrors.catatan_checkin_kerja ? 'failed' : ''"
              @input="v=>values.catatan_checkin_kerja=v" :hints="formErrors.catatan_checkin_kerja" :check="false"
              label="" placeholder="Catatan Check In Istirahat" />
          </div>
          <div class="flex flex-col">
            <label class="font-semibold">On Scope<span class="text-red-500 space-x-0 pl-0">*</span></label>
            <div class="flex py-2 gap-2">
              <input :disabled="!actionText" type="radio" :value="true" v-model="values.checkin_kerja_on_scope" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"/>
              <label for="ya">Ya</label>
              <div class="w-10 " />
              <input :disabled="!actionText" type="radio" v-model="values.checkin_kerja_on_scope" :value="false" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"/>
              <label for="tidak">Tidak</label>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-4">
          ...
          <!-- END COLUMN -->
              ...
              <div>
            <label class="font-semibold">Last Update<span class="text-red-500 space-x-0 pl-0"></span></label>
            <FieldX :bind="{ readonly: !actionText }" label="" class="w-full py-2 !mt-0"
              :value="values.last_editor_nama" :errorText="formErrors.last_editor_nama ? 'failed' : ''"
              @input="v=>values.last_editor_nama=v" :hints="formErrors.last_editor_nama" :check="false" label=""
              placeholder="Catatan Check Out" />
          </div>
      </div>
