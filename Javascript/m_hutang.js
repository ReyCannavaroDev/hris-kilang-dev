...
// ------------------------------ PERSIAPAN
const endpointApi = '/m_hutang_kary'
onBeforeMount(() => {
  document.title = 'Master Hutang'
})
...
const landing = reactive({
  ...
  api: {
    ...
    params: {
      simplest: true,
      view_debt: true,
      searchfield: 'this.id, m_kary.nama_depan, jenis_potongan.value, this.total_hutang, this.is_active, this.total_dibayar, this.total_hutang_jenis, this.sisa_hutang, this.tanggal',
    },
    onsuccess(response) {
  ...
  {
    field: 'jenis_potongan.value',
    headerName: 'Jenis Pinjaman ',
    filter: true,
    sortable: true,
    flex: 1,
    filter: 'ColFilter',
  ...
  {
    field: 'total_hutang_jenis',
    headerName: 'Total Pinjaman ',
    filter: true,
    sortable: true,
    flex: 1,
    filter: 'ColFilter',
  ...
  },
  {
    ...
    cellClass: ['border-r', '!border-gray-200', 'justify-center'],
     cellRenderer: ({ value }) => {
      return value === true
        ? `<span class="text-green-500 rounded-md text-xs font-medium px-4 py-1 inline-block uppercase">Aktif</span>`
        : `<span class="text-red-500 rounded-md text-xs font-medium px-4 py-1 inline-block uppercase">Non Aktif</span>`
    }
  },
  ]
})
