...
const onGenerate = async () => {
  ...
  tempGet.push(`size=${values.size || 'A4'}`)
  tempGet.push(`orientation=${values.orientation || 'portrait'}`)

  const paramsGet = tempGet.join("&")
  if (values.tipe?.toLowerCase() !== 'html') {
    exportHtml.value = false
    window.open(`${store.server.url_backend}/web/print_slip` + '?' + paramsGet)
  } else {
    await fetch(`${store.server.url_backend}/web/print_slip` + '?' + paramsGet, {
      headers: {
        'Content-Type': 'html',
      },
    })
