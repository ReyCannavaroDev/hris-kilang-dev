...
class default_users extends \App\Models\BasicModels\default_users
{
    ...
    public function transformRowData(array $row)
    {
        ...
        if ($req->from == "role_access") {
            $object["is_superadmin"] = m_role_access::join(
                "m_role as r",
                "r.id",
                "m_role_access.m_role_id"
            )
                ->where("m_role_access.user_id", $row["id"])
                ->pluck("is_superadmin")
                ->first();
            if ($req->detail) {
                $object["detail"] = m_role_access::join(
                    "m_role as r",
                    "r.id",
                    "m_role_access.m_role_id"
                )
        ...
        if ($req->withKary) {
            ...
            $object["dir"] = @m_dir::find($kary->m_dir_id)->nama;
        }
        $object["atasan"] = m_kary::where("id", @$row["m_kary.atasan_id"] ?? 0)
            ->pluck("nama_lengkap")
            ->first();

        if (app()->request->header("Source") === "mobile") {
            $data = \DB::select("select public.employee_attendance(?,?)", [
                Date("Y-m-d"),
                $row["m_kary_id"] ?? 0,
            ]);
            $data = json_decode($data[0]->employee_attendance);
            $object["m_kary.sisa_cuti_satu_hari"] =
                $data->sisa_cuti_satu_hari ?? 0;
            $object["m_kary.sisa_cuti_setengah_hari"] =
                $data->sisa_cuti_setengah_hari ?? 0;
            $object["m_kary.cuti_satu_hari"] = $data->cuti_satu_hari ?? 0;
            $object["m_kary.cuti_setengah_hari"] =
                $data->cuti_setengah_hari ?? 0;
    ...
    public function custom_getHBDKaryawan($request)
    {
        try {
            ...
            $karyawanHariIni = m_kary::leftJoin(
                "default_users",
                "m_kary.id",
                "=",
                "default_users.m_kary_id"
            )
            ...
            return response()->json(
                [
                    "message" =>
                        "Terjadi kesalahan saat mendapatkan data karyawan yang ulang tahun hari ini",
                    "error" => $e->getMessage(),
                ],
                500
            );
    ...
    public function createBefore($model, $arrayData, $metaData, $id = null)
    {
        ...
        $checkNIK = $model->where("m_kary_id", req("m_kary_id"))->exists();
        if ($checkNIK) {
            return ["errors" => ["Karyawan sudah digunakan"]];
        }
    ...
    {
        $kary = m_kary::whereRaw(
            "m_kary.id not in(select u.m_kary_id from default_users u where u.m_kary_id is not null)"
        )
            ->limit(200)
            ->get();
    ...
    public function custom_reset_password($req)
    {
        ...
        if ($req->email && $req->username) {
            \DB::table("default_users")
                ->where("id", $req->id_user ?? auth()->user()->id)
                ->update([
                    "username" => $req->username,
                    "email" => $req->email,
                    "updated_at" => \Carbon::now(),
                    "updated_id" => auth()->id(),
                    "no_break_needed" => $req->no_break_needed,
        ...
        // if(req("no_break_needed")){
        //     \DB::table("default_users")
        //         ->where("id", $req->id_user ?? auth()->user()->id)
        //         ->update([
        //             "no_break_needed" => req("password"),
        //         ]);
        // }
    ...
    {
        $kary_id = default_users::where("id", auth()->user()->id)
            ->pluck("m_kary_id")
            ->first();
        return $model->whereRaw(
            "default_users.id = ? or default_users.m_kary_id in (select k.id from default_users u join m_kary k on k.id = u.m_kary_id where k.atasan_id = ?)",
            [auth()->user()->id ?? 0, $kary_id]
        );
    }

    public function public_generateQRCode(Request $request)
    {
        ...
        $response = response($image, 200)->header("Content-Type", "image/png");
