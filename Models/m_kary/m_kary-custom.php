...
class m_kary extends \App\Models\BasicModels\m_kary
{
    ...
    public function m_hutang_kary() :\HasMany
    {
        return $this->hasMany('App\Models\BasicModels\m_hutang_kary', 'm_kary_id', 'id');
    }

     public function t_cuti() :\HasMany
    {
        return $this->hasMany('App\Models\BasicModels\t_cuti', 'm_kary_id', 'id');
    }
    ...
    public function createBefore($model, $arrayData, $metaData, $id = null)
    {
        $newArrayData = array_merge($arrayData, [
            ...
            "nama_belakang" => $arrayData['nama_belakang'] ?? ""
        ]);
        return [
            "model" => $model,
            "data" => $newArrayData,
    ...
    public function transformRowData( array $row )
    {
        ...
        if(app()->request->cuti_uang)
        {
            $data = \DB::select("select public.employee_attendance(?,?)",[Date('Y-m-d'),$row['id'] ??0]);
            $data = json_decode($data[0]->employee_attendance);
            // dd($data->sisa_cuti_satu_hari);
            $object['sisa_cuti_satu_hari'] = $data->sisa_cuti_satu_hari;

            $kary_salary = t_kary_salary::whereHas('t_kary_salary_det', function($q){
                $q->where('keterangan', 'Gaji Pokok');
            })
            ->where('m_kary_id', $row['id'])
            ->first();

            $object['salary'] = $kary_salary?->t_kary_salary_det[0]->nominal;
            $object['salary_type'] = $kary_salary?->tipe_perhitungan;
        }

        // if(app()->request->pinjaman)
        // {
        //     $pinjaman = m_hutang_kary::where('m_kary_id', $row['id'])
        ...
        //     $t_potongan = t_potongan::where('jenis_potongan_id', $pinjaman->jenis_potongan_id)
        //     ->where('m_kary_id', $row['id'])->first();

        //     $paid = $t_final_gaji_det_rincian::whereHas('t_final_gaji_det', function($q){
        //         $q->where('m_kary_id', $row['id']);
        //     })->where('t_potongan_id', $t_potongan->id)
        //     ->sum('value');

        //     dd($paid);

        //     $object['detail_pinjaman'] = $pinjaman;
    ...
    public function scopeCutiUang($model)
    {    
        return $model
        ->join('t_kary_salary', 't_kary_salary.m_kary_id', '=', 'm_kary.id')
        ->join('t_kary_salary_det', 't_kary_salary_det.t_kary_salary_id', '=', 't_kary_salary.id')
        ->where('t_kary_salary_det.keterangan', 'Gaji Pokok')
        ->select('m_kary.*', 't_kary_salary_det.nominal as gaji_pokok', 't_kary_salary.tipe_perhitungan as tipe_gaji');
    }

    public function custom_listPinjaman()
    {

        $nama = app()->request->nama ?? null;

        $kary = $this->whereHas('m_hutang_kary')
            ->when($nama, function ($q) use ($nama) {
                $q->where('m_kary.nama_lengkap', 'ILIKE', "%{$nama}%");
            })
            ->with(['m_hutang_kary' => function($q) {
                $q->select(
                    'm_kary_id',
                    'jenis_potongan_id',
                    DB::raw('SUM(total_hutang) as total_hutang'),
                    DB::raw('MAX(tanggal) as tanggal_hutang_terakhir'),
                )
                ->groupBy('jenis_potongan_id', 'm_kary_id');
            }])
            ->select('id', 'nama_depan', 'nama_lengkap')
            ->get()
            ->map(function($item) {
                foreach ($item->m_hutang_kary as $hutang) {
                    ...
                    $hutang->paid = (int) $paid;
                    $hutang->sisa_hutang = max(0, $hutang->total_hutang - $paid);
                }
                return $item;
            });
     ...
     public function scopePinjaman($model)
    {

        $nama = app()->request->nama ?? null;

        $model->whereHas('m_hutang_kary')
            ->when($nama, function ($q) use ($nama) {
                $q->where('m_kary.nama_lengkap', 'ILIKE', "%{$nama}%");
            })
            ->with(['m_hutang_kary' => function($q) {
            ...
            ->select('m_kary.id', 'm_kary.nama_depan', 'm_kary.nama_lengkap')
            ->get()
            ->map(function($item) {
                foreach ($item->m_hutang_kary as $hutang) {
                    $t_potongan = t_potongan::where('m_kary_id', $item->id)
