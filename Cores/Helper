    ...
    public function approvalOustanding()
{
    ...
    $data = generate_approval::selectRaw("
            ...
            m_kary.nama_lengkap as nama_karyawan
        ")
        ->leftJoin('default_users', 'default_users.id', 'generate_approval.creator_id')
        ->leftJoin('t_cuti', 't_cuti.id', 'generate_approval.trx_id')
        ->leftJoin('m_kary', 't_cuti.m_kary_id', 'm_kary.id')
        ...
        ->search([
            'generate_approval.trx_name',
            'generate_approval.nomor',
            'generate_approval.trx_date',
            'generate_approval.trx_nomor',
            'default_users.name',
            'm_kary.nama_lengkap'
        ])
